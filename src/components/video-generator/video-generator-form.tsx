...
import { VideoSchema, videoSchema } from '@/lib/schema';
import { useUser } from '@clerk/nextjs';
import { zodResolver } from '@hookform/resolvers/zod';
import { useQuery, useAction } from 'convex/react';
import { X, CrownIcon, Loader2, UploadIcon } from 'lucide-react';
...
export const VideoGeneratorForm = () => {
  ...
  const generateVideo = useAction(
    api.video_generations.actions.generate
  );

  const form = useForm<VideoSchema>({
    resolver: zodResolver(videoSchema),
    defaultValues: {
      model: VIDEO_GENERATION_MODELS[0].value,
      aspectRatio: VIDEO_ASPECT_RATIOS[0].value,
      prompt: '',
  ...
  async function onSubmit(values: VideoSchema) {
  ...
  const handleObjectImageSelect = async (
    ...
    if (file.size > MAX_FILE_SIZE) {
      toast.error('MAX VIDEO SIZE IS 5MB');
      return;
    }
